...
router.get("/recommendations", async (req, res) => {
  ...
  try {
    const cleanKwd = title.replace(/[【】「」()!！?？\[\]]/g, ' ').replace(/\s+/g, ' ').trim();
    ...
    let rawList = [...(topicRes.items || []), ...(channelRes.items || []), ...(relatedRes.items || [])];
    ...
    for (const item of rawList) {
      ...
      const normalized = item.title.toLowerCase().replace(/\s+/g, '').replace(/official|lyrics|mv|musicvideo|video|公式|実況|解説/g, '');
...
module.exports = { router, apiListCache, fetchWithTimeout };
